...
description: Archive a completed and strictly validated OpenSpec change. Use only after implementation is complete.
...
**Input**: Optionally specify a change name. If omitted and the change cannot be inferred unambiguously, run `openspec list --json` and use the **question tool** to let the user select an active change.
